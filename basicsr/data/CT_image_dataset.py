...
class Dataset_CTDenoising(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        print("Before filtering, number of target images: ", len(self.target_path))
        print("Before filtering, number of input images: ", len(self.input_path))

        self.target_path = [f for f in self.target_path if "1mm" in f and "B30" in f]
        self.input_path = [f for f in self.input_path if "1mm" in f and "B30" in f]

        print("After filtering, number of target images: ", len(self.target_path))
        print("After filtering, number of input images: ", len(self.input_path))
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            ...
            # random crop
            img_gt, img_lq = paired_random_crop(img_gt, img_lq, gt_size, 1, "none")
            # flip, rotation
